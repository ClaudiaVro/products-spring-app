...
@Service public class ProductService {
    ...
    public void uploadProductImage(Long id, MultipartFile file) {
        ...
        List<String> contentTypes = Arrays.asList(ContentType.IMAGE_JPEG.getMimeType(), ContentType.IMAGE_PNG.getMimeType(),
        ...
        if (productOpt.isPresent()) {
            ...
            String path = String.format("%s/%s", BucketName.PRODUCT_IMAGES.getBucketName(), product.getId());
            try {
                amazonFileStoreService.save(path, file.getOriginalFilename(), Optional.of(metadata), file.getInputStream());
            }
            catch (IOException e) {
